...
class Schema:
    ...
    def _instantiate_helper(formula: Formula,
        ...
        if is_binary(formula.root):
            new_first = Schema._instantiate_helper(formula.first,
                                                   constants_and_variables_instantiation_map,
                                                   relations_instantiation_map, bound_variables)
            new_second = Schema._instantiate_helper(formula.second,
                                                    constants_and_variables_instantiation_map,
                                                    relations_instantiation_map, bound_variables)
            ...
            rel_var = formula.variable
            if formula.variable in constants_and_variables_instantiation_map:
                rel_var = constants_and_variables_instantiation_map[formula.variable].root
            new_bounded_variables = set(bound_variables)
            new_bounded_variables.add(rel_var)
            new_formula = Schema._instantiate_helper(formula.predicate,
                                                     constants_and_variables_instantiation_map,
                                                     relations_instantiation_map,
                                                     new_bounded_variables)
            return Formula(formula.root, rel_var, new_formula)
        elif formula.root == '=' or (
                is_relation(formula.root) and formula.root not in relations_instantiation_map):
            return formula.substitute(constants_and_variables_instantiation_map, set())
        elif formula.root in relations_instantiation_map:
            if len(formula.arguments) == 0:
                rel_formula = relations_instantiation_map[formula.root]
                for var in formula.free_variables():
                    if var not in rel_formula.free_variables():
                ...
                new_arg = formula.arguments[0].substitute(constants_and_variables_instantiation_map,
                                                          set())
                for var in relations_instantiation_map[formula.root].free_variables():
                    if var in bound_variables:
                        raise (Schema.BoundVariableError(var, formula.root))
                rel_map = {'_': new_arg}
                try:
                    return relations_instantiation_map[formula.root].substitute(rel_map, set())
    ...
    def instantiate(self, instantiation_map: InstantiationMap) -> \
        ...
        for key in instantiation_map:
            if is_constant(key):
                constants_and_vars_map[key] = instantiation_map[key]
            elif is_variable(key):
                constants_and_vars_map[key] = Term(instantiation_map[key])
            elif is_relation(key):
                relations_map[key] = instantiation_map[key]
        try:
            return Schema._instantiate_helper(self.formula, constants_and_vars_map, relations_map,
                                              set())
...
class Proof:
    ...
    class AssumptionLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            if self.assumption not in assumptions:
                return False
            if not self.assumption.instantiate(self.instantiation_map) == self.formula:
                return False
            return True
    ...
    class MPLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            assert line_number < len(lines) and lines[line_number] is self
            # Task 9.6
            if self.conditional_line_number >= line_number or self.antecedent_line_number >= line_number:
                return False
            rel_formula = Formula('->', lines[self.antecedent_line_number].formula, self.formula)
            return rel_formula == lines[self.conditional_line_number].formula
    ...
    class UGLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            assert line_number < len(lines) and lines[line_number] is self
            # Task 9.7
            if self.predicate_line_number > line_number or not is_quantifier(self.formula.root) \
                    or not is_variable(self.formula.variable) or self.formula.root != "A":
                return False
    ...
    class TautologyLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            assert line_number < len(lines) and lines[line_number] is self
            # Task 9.9
            new_form, new_dict = self.formula.propositional_skeleton()
            return is_propositional_tautology(new_form)
...
def axiom_specialization_map_to_schema_instantiation_map(
    ...
    for k, v in propositional_specialization_map.items():
        new_dict[k.capitalize()] = Formula.from_propositional_skeleton(v, substitution_map)
    return new_dict
...
def prove_from_skeleton_proof(formula: Formula,
    ...
    for line in skeleton_proof.lines:
        line_form = Formula.from_propositional_skeleton(line.formula, substitution_map)
        if line.rule == MP:
            lines.append(
                Proof.MPLine(line_form,
                             line.assumptions[0], line.assumptions[1]))
        else:
            lines.append(Proof.AssumptionLine(line_form, PROPOSITIONAL_AXIOM_TO_SCHEMA[line.rule],
                                              axiom_specialization_map_to_schema_instantiation_map(
                                                  PropositionalInferenceRule.formula_specialization_map(
                                                      line.rule.conclusion, line.formula),
                                                  substitution_map)))
    return Proof(PROPOSITIONAL_AXIOMATIC_SYSTEM_SCHEMAS, lines[-1].formula, lines)
...
def prove_tautology(tautology: Formula) -> Proof:
    ...
    # Task 9.12
    skeleton_form, skeleton_map = tautology.propositional_skeleton()
    taut_proof = prove_propositional_tautology(skeleton_form)
    return prove_from_skeleton_proof(tautology, taut_proof, skeleton_map)
