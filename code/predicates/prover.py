...
class Prover:
    ...
    def add_universal_instantiation(self, instantiation: Union[Formula, str],
        ...
        # Task 10.1
        rel_quantifier = self._lines[line_number].formula.variable
        predicate_repr = self._lines[line_number].formula.predicate.__repr__()
        var_free_predicate = Prover.replace_var_in_str(predicate_repr, rel_quantifier, '_')
        instantiation_repr = instantiation
        if isinstance(instantiation, Formula):
            instantiation_repr = instantiation.__repr__()
        UI_line = '(' + self._lines[
            line_number].formula.__repr__() + '->' + instantiation_repr + ')'
        instantiation_map = {'c': term, 'R': var_free_predicate}
        if not rel_quantifier == 'x':
            instantiation_map['x'] = rel_quantifier
        self.add_instantiated_assumption(UI_line, Prover.UI, instantiation_map)
        UI_line_index = len(self._lines) - 1
        return self.add_mp(instantiation, line_number, UI_line_index)

    @staticmethod
    def replace_var_in_str(predicate_repr, rel_quantifier, replace):
        """
        a method replacing a rel_quantifier var in the predicate_repr string with replace object.
        :param predicate_repr: a string to replace the rel_quantifier var in
        :param rel_quantifier: the var to replace
        :param replace: the object to replace the var with
        :return: the new string after switch.
        """
        pred_form = Formula.parse(predicate_repr)
        if is_variable(pred_form.root):
            if predicate_repr == rel_quantifier:
                return replace
            return predicate_repr
        if is_constant(pred_form.root):
            return predicate_repr
        if is_unary(pred_form.root):
            return pred_form.root + Prover.replace_var_in_str(pred_form.first.__repr__(),
                                                              rel_quantifier, replace)
        if is_binary(pred_form.root):
            first = Prover.replace_var_in_str(pred_form.first.__repr__(), rel_quantifier, replace)
            second = Prover.replace_var_in_str(pred_form.second.__repr__(), rel_quantifier, replace)
            return "(" + first + pred_form.root + second + ")"
        if is_equality(pred_form.root):
            first = Prover.replace_for_term(pred_form.arguments[0].__repr__(), rel_quantifier,
                                            replace)
            second = Prover.replace_for_term(pred_form.arguments[1].__repr__(), rel_quantifier,
                                             replace)
            return first + "=" + second
        if is_quantifier(pred_form.root):
            if pred_form.variable == rel_quantifier:
                return predicate_repr
            return pred_form.root + pred_form.variable + '[' + Prover.replace_var_in_str(
                pred_form.predicate.__repr__(), rel_quantifier,
                replace) + ']'
        if is_relation(pred_form.root):
            args = pred_form.root + "("
            for term in pred_form.arguments:
                if term.__repr__() == rel_quantifier:
                    args += replace + ','
                else:
                    args += term.__repr__() + ','
            if args[-1] == ',':
    ...
    @staticmethod
    def replace_for_term(predicate_repr, rel_quantifier, replace):
        """
        a method replacing a rel_quantifier var in the predicate_repr string with replace object.
        :param predicate_repr: a string to replace the rel_quantifier var in
        :param rel_quantifier: the var to replace
        ...
        :return: the new string after switch.
        """
        pred_lst = [char for char in predicate_repr]
        new_lst = []
        run_to = 0
        for i in range(len(pred_lst)):
            if run_to > i:
                continue
            is_identical = True
            if pred_lst[i] == rel_quantifier[0]:
                for j in range(len(rel_quantifier)):
                    if pred_lst[i + j] != rel_quantifier[j]:
                        is_identical = False
                        break
                if not is_identical:
                    new_lst.append(pred_lst[i])
                    continue
                if len(rel_quantifier) == 1:
                    if i == len(pred_lst) - 1 or not pred_lst[i + 1].isdigit():
                        new_lst.append(replace)
                        continue
                    new_lst.append(pred_lst[i])
                else:
                    new_lst.append(replace)
                    run_to = i + len(rel_quantifier)
            else:
                new_lst.append(pred_lst[i])
        return "".join(new_lst)
    ...
    def add_tautological_implication(self, implication: Union[Formula, str],
        ...
        for line_number in line_numbers:
            assert line_number < len(self._lines)
        # Task 10.2
        rel_line_indexes = list(line_numbers)
        implication_formula = implication
        if not isinstance(implication, Formula):
            implication_formula = Formula.parse(implication)
        tautology_formula = Formula('->', self._lines[rel_line_indexes[-1]].formula,
                                    implication_formula)
        for i in reversed(rel_line_indexes[:-1]):
            tautology_formula = Formula('->', self._lines[i].formula, tautology_formula)
        tautology_index = self.add_tautology(tautology_formula)
        rel_tautology_formula = tautology_formula
        rel_index_for_mp = tautology_index
        for i in rel_line_indexes:
            rel_index_for_mp = self.add_mp(rel_tautology_formula.second, i, rel_index_for_mp)
            rel_tautology_formula = rel_tautology_formula.second
        return rel_index_for_mp
    ...
    def add_existential_derivation(self, consequent: Union[Formula, str],
        ...
        # Task 10.3
        predicate_repr = self._lines[line_number1].formula.predicate.__repr__()  # for es line
        rel_quantifier = self._lines[line_number1].formula.variable
        ug_formula_str = 'A' + rel_quantifier + '[' + self._lines[
            line_number2].formula.__repr__() + ']'
        ug_formula = Formula.parse(ug_formula_str)
        ug_index = self.add_ug(ug_formula_str, line_number2)
        rel_consequent = consequent
        if not isinstance(consequent, Formula):
            rel_consequent = Formula.parse(consequent)
        es_assumption_formula = Formula('->',
                                        Formula('&', ug_formula, self._lines[line_number1].formula),
                                        rel_consequent)
        instantiation_map = {'Q': rel_consequent.__repr__()}
        var_free_predicate = predicate_repr.replace(rel_quantifier, '_')
        instantiation_map['R'] = var_free_predicate
        if not rel_quantifier == 'x':
            instantiation_map['x'] = rel_quantifier
        es_index = self.add_instantiated_assumption(es_assumption_formula, Prover.ES,
                                                    instantiation_map)
        line_numbers = {line_number1, ug_index, es_index}
        return self.add_tautological_implication(consequent, line_numbers)
    ...
    def add_flipped_equality(self, flipped: Union[Formula, str],
        ...
        # Task 10.6
        new_map = dict()
        new_map["R"] = Formula.parse("_=" + equality.arguments[0].__repr__())
        new_map["c"] = equality.arguments[0]
        new_map["d"] = equality.arguments[1]
        me_line = Formula.parse(
            "(" + equality.__repr__() + "->" + '(' + equality.arguments[0].__repr__() + "=" +
            equality.arguments[
                0].__repr__() + "->" + equality.arguments[1].__repr__() + "=" + equality.arguments[
                0].__repr__() + "))")
        me_index = self.add_instantiated_assumption(me_line, self.ME, new_map)
        rx_line = Formula.parse(
            equality.arguments[0].__repr__() + "=" + equality.arguments[0].__repr__())
        new_map = dict()
        new_map["c"] = equality.arguments[0]
        rx_index = self.add_instantiated_assumption(rx_line, self.RX, new_map)
        mp_form = Formula.parse('(' + equality.arguments[0].__repr__() + "=" + equality.arguments[
            0].__repr__() + "->" + equality.arguments[1].__repr__() + "=" + equality.arguments[
                                    0].__repr__() + ")")
        first_mp = self.add_mp(mp_form, line_number, me_index)
        return self.add_mp(flipped, rx_index, first_mp)
    ...
    def add_free_instantiation(self, instantiation: Union[Formula, str],
        ...
        # Task 10.7
        unqunt_line = self._lines[line_number].formula
        new_map = dict()
        switch_index = self.switch_free_vars(line_number, new_map, substitution_map, unqunt_line,
                                             True)
        return self.switch_free_vars(switch_index, {}, new_map, self._lines[switch_index].formula,
                                     False)

    def switch_free_vars(self, line_number, new_map, substitution_map, unqunt_line, not_z):
        """
        a method switching the free vars in Z terms , and then can be used to switch Z to a different vars
        :param line_number: the number of line that we switch
        :param new_map: a new map created in this method
        :param substitution_map: a map to switch the vars accordingly
        :param unqunt_line: the line before switching
        :param not_z: whether we switch normally, or we switch by z.
        :return: the line of the last switched.
        """
        switch_index = line_number
        for key in substitution_map:
            if not_z:
                var = next(fresh_variable_name_generator)
                new_map[var] = substitution_map[key]
                term_var = Term.parse(var)
            else:
                var = substitution_map[key]
                term_var = var
            ug_index = self.add_ug(Formula("A", key, unqunt_line), switch_index)
            unqunt_line = unqunt_line.substitute({key: term_var}, {})
            switch_index = self.add_universal_instantiation(unqunt_line,
                                                            ug_index, var)
        return switch_index
    ...
    def add_substituted_equality(self, substituted: Union[Formula, str],
        ...
        # Task 10.8
        my_line = self._lines[line_number].formula
        new_map = dict()
        left_equal = Prover.replace_for_term(parametrized_term.__repr__(), "_",
                                             my_line.arguments[0].__repr__())
        new_map["R"] = Formula.parse(left_equal + "=" + parametrized_term.__repr__())
        new_map["c"] = my_line.arguments[0]
        new_map["d"] = my_line.arguments[1]
        second_right = Prover.replace_for_term(parametrized_term.__repr__(), "_",
                                               my_line.arguments[1].__repr__())
        rx_line = left_equal + "=" + left_equal
        me_line = Formula.parse(
            "(" + my_line.__repr__() + "->" + '(' + rx_line + "->" + left_equal + "=" + second_right + "))")
        me_index = self.add_instantiated_assumption(me_line, self.ME, new_map)
        new_map = dict()
        new_map["c"] = left_equal
        rx_index = self.add_instantiated_assumption(Formula.parse(rx_line), self.RX, new_map)
        mp_form = Formula.parse('(' + rx_line + "->" + left_equal + "=" + second_right + ")")
        first_mp = self.add_mp(mp_form, line_number, me_index)
        return self.add_mp(substituted, rx_index, first_mp)
    ...
    def _add_chaining_of_two_equalities(self, line_number1: int,
        ...
        # Task 10.9.1
        flipped_form = Formula.parse(
            equality1.arguments[1].__repr__() + "=" + equality1.arguments[0].__repr__())
        flipped_line1 = self.add_flipped_equality(flipped_form, line_number1)
        new_map = dict()
        new_map['R'] = Formula.parse("_=" + equality2.arguments[1].__repr__())
        new_map['c'] = equality1.arguments[1]
        new_map['d'] = equality1.arguments[0]
        conclustion = equality1.arguments[0].__repr__() + '=' + equality2.arguments[1].__repr__()
        me_line = Formula.parse('(' + flipped_form.__repr__() + "->" + '(' + equality1.arguments[
            1].__repr__() + '=' + equality2.arguments[1].__repr__() + "->" + conclustion + "))")
        me_index = self.add_instantiated_assumption(me_line, self.ME, new_map)
        mp_form = Formula.parse('(' + equality1.arguments[1].__repr__() + '=' + equality2.arguments[
            1].__repr__() + "->" + conclustion + ")")
        first_mp = self.add_mp(mp_form, flipped_line1, me_index)
        return self.add_mp(Formula.parse(conclustion), line_number2, first_mp)
    ...
    def add_chained_equality(self, chained: Union[Formula, str],
        ...
        # Task 10.9.2
        new_num = line_numbers[0]
        for i in range(1, len(line_numbers)):
            new_num = self._add_chaining_of_two_equalities(new_num, line_numbers[i])
        return new_num
