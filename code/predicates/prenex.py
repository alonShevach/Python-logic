...
def to_prenex_normal_form_from_uniquely_named_variables(formula: Formula) -> \
    ...
    if is_binary(formula.root):
        first, proof1 = to_prenex_normal_form_from_uniquely_named_variables(formula.first)
        second, proof2 = to_prenex_normal_form_from_uniquely_named_variables(formula.second)
        proof_line1 = prover.add_proof(equivalence_of(formula.first, first), proof1)
        proof_line2 = prover.add_proof(equivalence_of(formula.second, second), proof2)
        binform, proof3 = pull_out_quantifications_across_binary_operator(Formula(formula.root, first, second))
        proof_line3 = prover.add_proof(equivalence_of(Formula(formula.root, first, second), binform), proof3)
        taut = prover.add_tautological_implication(equivalence_of(formula, Formula(formula.root, first, second)),
                                                   {proof_line1, proof_line2})
        prover.add_tautological_implication(equivalence_of(formula, binform), {taut, proof_line3})
        return binform, prover.qed()
    if is_unary(formula.root):
        first, proof1 = to_prenex_normal_form_from_uniquely_named_variables(formula.first)
        proof_line0 = prover.add_proof(equivalence_of(formula.first, first), proof1)
        second, proof2 = pull_out_quantifications_across_negation(Formula(formula.root, first))
        proof_line1 = prover.add_proof(equivalence_of(Formula(formula.root, first), second), proof2)
        taut = prover.add_tautological_implication(equivalence_of(Formula(formula.root, first), formula), {proof_line0})
        prover.add_tautological_implication(equivalence_of(formula, second), {taut, proof_line1})
        return second, prover.qed()
    if is_quantifier(formula.root):
        first, proof1 = to_prenex_normal_form_from_uniquely_named_variables(formula.predicate)
        proof_line0 = prover.add_proof(equivalence_of(formula.predicate, first), proof1)
        var = formula.variable
        equil1 = equivalence_of(formula, Formula(formula.root, formula.variable, first))
        if formula.root == "A":
            quantifier = ADDITIONAL_QUANTIFICATION_AXIOMS[14]
        else:
            quantifier = ADDITIONAL_QUANTIFICATION_AXIOMS[15]
        for_mp = prover.add_instantiated_assumption(
            Formula("->", equivalence_of(formula.predicate, first),
                    equil1), quantifier, {'x': var, 'y': var, 'R': formula.predicate.substitute({var: Term('_')}),
                                          'Q': first.substitute({var: Term('_')})})
        mp = prover.add_mp(equil1, proof_line0, for_mp)
        return Formula(formula.root, var, first), prover.qed()
    prover.add_tautology(equivalence_of(formula, formula))
    return formula, prover.qed()
...
def to_prenex_normal_form(formula: Formula) -> Tuple[Formula, Proof]:
    ...
    # Task 11.10
    prover = Prover(Prover.AXIOMS.union(ADDITIONAL_QUANTIFICATION_AXIOMS))
    new_form, proof1 = uniquely_rename_quantified_variables(formula)
    final_form, proof2 = to_prenex_normal_form_from_uniquely_named_variables(new_form)
    proof_line1 = prover.add_proof(proof1.conclusion, proof1)
    proof_line2 = prover.add_proof(proof2.conclusion, proof2)
    prover.add_tautological_implication(equivalence_of(formula, final_form), {proof_line1, proof_line2})
    return final_form, prover.qed()
