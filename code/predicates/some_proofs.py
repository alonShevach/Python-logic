...
def homework_proof(print_as_proof_forms: bool = False) -> Proof:
    ...
    step1 = prover.add_tautology(
        '(((Homework(x)&Fun(x))->Ex[(Homework(x)&Fun(x))])->(~Ex[(Homework(x)&Fun(x))]->~(Homework(x)&Fun(x))))')
    step2 = prover.add_instantiated_assumption('((Homework(x)&Fun(x))->Ex[(Homework(x)&Fun(x))])',
                                               Prover.EI, {'R': '(Homework(_)&Fun(_))', 'x': 'x', 'c': 'x'})
    step3 = prover.add_mp('(~Ex[(Homework(x)&Fun(x))]->~(Homework(x)&Fun(x)))', step2, step1)
    step4 = prover.add_assumption('~Ex[(Homework(x)&Fun(x))]')
    step5 = prover.add_mp('~(Homework(x)&Fun(x))', step4, step3)
    step6 = prover.add_tautology('(~(Homework(x)&Fun(x))->((Homework(x)&Reading(x))->(Reading(x)&~Fun(x))))')
    step7 = prover.add_mp('((Homework(x)&Reading(x))->(Reading(x)&~Fun(x)))', step5, step6)
    ...
    step10 = prover.add_tautological_implication('((Homework(x)&Reading(x))->Ex[(Reading(x)&~Fun(x))])', {step7, step9})
    step9 = prover.add_existential_derivation('Ex[(Reading(x)&~Fun(x))]', step8, step10)
    return prover.qed()
...
def unique_zero_proof(print_as_proof_forms: bool = False) -> Proof:
    ...
    step1 = prover.add_substituted_equality("plus(minus(a),plus(a,c))=plus(minus(a),a)", step0, "plus(minus(a),_)")
    step2 = prover.add_assumption('plus(minus(x),x)=0')
    step3 = prover.add_free_instantiation("plus(minus(a),a)=0", step2, {'x': 'a'})
    step4 = prover.add_chained_equality("plus(minus(a),plus(a,c))=0", [step1, step3])
    step5 = prover.add_flipped_equality("0=plus(minus(a),plus(a,c))", step4)
    step6 = prover.add_assumption("plus(plus(x,y),z)=plus(x,plus(y,z))")
    step7 = prover.add_flipped_equality("plus(x,plus(y,z))=plus(plus(x,y),z)", step6)
    step8 = prover.add_free_instantiation("plus(minus(a),plus(a,c))=plus(plus(minus(a),a),c)", step7,
                                          {'x': "minus(a)", 'y': 'a', 'z': 'c'})
    step9 = prover.add_substituted_equality("plus(plus(minus(a),a),c)=plus(0,c)", step3, "plus(_,c)")
    step10 = prover.add_assumption("plus(0,x)=x")
    step11 = prover.add_free_instantiation("plus(0,c)=c", step10, {'x': 'c'})
    step12 = prover.add_chained_equality("0=c", [step5, step8, step9, step11])
    step13 = prover.add_flipped_equality("c=0", step12)
    return prover.qed()
...
def multiply_zero_proof(print_as_proof_forms: bool = False) -> Proof:
    ...
    # Task 10.11
    step0 = prover.add_assumption('plus(minus(x),x)=0')
    step1 = prover.add_flipped_equality("0=plus(minus(x),x)", step0)
    step2 = prover.add_assumption('plus(x,y)=plus(y,x)')
    step3 = prover.add_free_instantiation("plus(minus(x),x)=plus(x,minus(x))", step2, {'x': "minus(x)", 'y': 'x'})
    step4 = prover.add_chained_equality("0=plus(x,minus(x))", [step1, step3])
    step5 = prover.add_assumption("times(x,1)=x")
    step6 = prover.add_substituted_equality("plus(times(x,1),minus(x))=plus(x,minus(x))", step5, "plus(_,minus(x))")
    step7 = prover.add_flipped_equality("plus(x,minus(x))=plus(times(x,1),minus(x))", step6)
    step8 = prover.add_assumption('plus(0,x)=x')
    step9 = prover.add_free_instantiation("plus(0,1)=1", step8, {'x': '1'})
    step10 = prover.add_substituted_equality("times(x,plus(0,1))=times(x,1)", step9, "times(x,_)")
    step11 = prover.add_substituted_equality("plus(times(x,plus(0,1)),minus(x))=plus(times(x,1),minus(x))", step10,
                                             "plus(_,minus(x))")
    step12 = prover.add_flipped_equality("plus(times(x,1),minus(x))=plus(times(x,plus(0,1)),minus(x))", step11)
    step13 = prover.add_assumption('times(x,plus(y,z))=plus(times(x,y),times(x,z))')
    step14 = prover.add_free_instantiation("times(x,plus(0,1))=plus(times(x,0),times(x,1))", step13,
                                           {'y': '0', 'z': '1'})
    step15 = prover.add_substituted_equality(
        "plus(times(x,plus(0,1)),minus(x))=plus(plus(times(x,0),times(x,1)),minus(x))", step14, "plus(_,minus(x))")
    step16 = prover.add_assumption('plus(plus(x,y),z)=plus(x,plus(y,z))')
    step17 = prover.add_free_instantiation(
        "plus(plus(times(x,0),times(x,1)),minus(x))=plus(times(x,0),plus(times(x,1),minus(x)))", step16,
        {'x': "times(x,0)", 'y': 'times(x,1)', 'z': "minus(x)"})
    step18 = prover.add_substituted_equality(
        "plus(times(x,0),plus(times(x,1),minus(x)))=plus(times(x,0),plus(x,minus(x)))", step6, "plus(times(x,0),_)")
    step19 = prover.add_flipped_equality("plus(x,minus(x))=0", step4)
    step20 = prover.add_substituted_equality("plus(times(x,0),plus(x,minus(x)))=plus(times(x,0),0)", step19,
                                             "plus(times(x,0),_)")
    step21 = prover.add_free_instantiation("plus(times(x,0),0)=plus(0,times(x,0))", step2,
                                           {'x': "times(x,0)", 'y': '0'})
    step22 = prover.add_free_instantiation("plus(0,times(x,0))=times(x,0)", step8, {'x': "times(x,0)"})
    step23 = prover.add_assumption('times(x,y)=times(y,x)')
    step24 = prover.add_free_instantiation("times(x,0)=times(0,x)", step23, {'x': 'x', 'y': '0'})
    step25 = prover.add_chained_equality("0=times(0,x)",
                                         [step4, step7, step12, step15, step17, step18, step20, step21, step22,
                                          step24])
    step26 = prover.add_flipped_equality("times(0,x)=0", step25)
    return prover.qed()
...
def peano_zero_proof(print_as_proof_forms: bool = False) -> Proof:
    ...
    step4 = prover.add_instantiated_assumption("(plus(0,x)=x->(s(plus(0,x))=s(plus(0,x))->s(plus(0,x))=s(x)))",
                                               prover.ME, {'c': "plus(0,x)", 'd': 'x', 'R': "s(plus(0,x))=s(_)"})
    step5 = prover.add_instantiated_assumption("s(plus(0,x))=s(plus(0,x))", prover.RX, {"c": "s(plus(0,x))"})
    step6 = prover.add_instantiated_assumption("(s(plus(0,x))=s(x)->(plus(0,s(x))=s(plus(0,x))->plus(0,s(x))=s(x)))",
                                               prover.ME, {'c': "s(plus(0,x))", 'd': 's(x)', 'R': "plus(0,s(x))=_"})
    ...
    step9 = prover.add_tautological_implication("(plus(0,x)=x->s(plus(0,x))=s(x))", {step4, step5})
    step10 = prover.add_tautological_implication("(s(plus(0,x))=s(x)->plus(0,s(x))=s(x))", {step6, step3})
    step11 = prover.add_tautological_implication("(plus(0,x)=x->plus(0,s(x))=s(x))", {step9, step10})
    step12 = prover.add_ug("Ax[(plus(0,x)=x->plus(0,s(x))=s(x))]", step11)
    step13 = prover.add_tautological_implication("(plus(0,0)=0&Ax[(plus(0,x)=x->plus(0,s(x))=s(x))])", {step1, step12})
    step14 = prover.add_mp("Ax[plus(0,x)=x]", step13, step8)
...
def russell_paradox_proof(print_as_proof_forms: bool = False) -> Proof:
    ...
    # Task 10.13
    step1 = prover.add_instantiated_assumption('Ey[Ax[((In(x,y)->~In(x,x))&(~In(x,x)->In(x,y)))]]', COMPREHENSION_AXIOM, {'R': '~In(_,_)'})
    step2 = prover.add_tautology('(((In(y,y)->~In(y,y))&(~In(y,y)->In(y,y)))->(z=z&~z=z))')
    step3 = prover.add_instantiated_assumption('(Ax[((In(x,y)->~In(x,x))&(~In(x,x)->In(x,y)))]->((In(y,y)->~In(y,y))&(~In(y,y)->In(y,y))))',
                                               Prover.UI, {'R':'((In(_,y)->~In(_,_))&(~In(_,_)->In(_,y)))', 'c':'y', 'x': 'x'})
    step4 = prover.add_tautological_implication('(Ax[((In(x,y)->~In(x,x))&(~In(x,x)->In(x,y)))]->(z=z&~z=z))', {step2, step3})
