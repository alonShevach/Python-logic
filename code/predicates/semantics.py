...
class Model(Generic[T]):
    ...
    def evaluate_term(self, term: Term,
        ...
        if is_constant(term.root):
            ...
            for arg in term.arguments:
                all_args_values += (self.evaluate_term(arg, assignment),)
            return self.function_meanings[term.root][all_args_values]
    ...
    def evaluate_helper(self, assignment, formula):
        if is_equality(formula.root):
            ...
            for arg in formula.arguments:
                all_args_values += (self.evaluate_term(arg, assignment),)
            return all_args_values in self.relation_meanings[formula.root]
            ...
            if formula.root == '&':
                ...
                return self.evaluate_formula(formula.first, assignment) == False or \
                       self.evaluate_formula(formula.second, assignment) == True
        else:
            if formula.root == 'E':
    ...
    def is_model_of(self, formulas: AbstractSet[Formula]) -> bool:
        ...
        for formula in formulas:
            ...
            tup_lst = []
            self.get_all_assignments(free_vars, tup_lst, [], 0)
            new_dict = []
            self.make_dict_from_lst(tup_lst, new_dict)
            for assign in new_dict:
                if not self.evaluate_formula(formula, assign):
                    return False
        return True

    def get_all_assignments(self, free_vars, tup_lst, temp_lst, i):
        """
        a helper method for the is_model_of method, this method creates all the different
        kinds of assignment possible for the given formula free variables.
        :param free_vars: the variables of the formula
        :param tup_lst: a list of tuples, the assignments
        :param temp_lst: a temporary list
        :param i: an index
        """
        if len(temp_lst) == len(free_vars):
            tup_lst.append(copy.copy(temp_lst))
            return
        for element in self.universe:
            temp_lst.append((free_vars[i], element))
            self.get_all_assignments(free_vars, tup_lst, temp_lst, i + 1)
            temp_lst.pop()
        return

    @staticmethod
    def make_dict_from_lst(tup_lst, new_dict):
        """
        a simple function that creates a dictonary from a given list of tuples
        :param tup_lst: a list of tuples
        :param new_dict: the new dict to update.
        """
        for lst in tup_lst:
            temp_dict = {}
            for k, v in lst:
                temp_dict[k] = v
            new_dict.append(copy.copy(temp_dict))